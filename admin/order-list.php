        ...
        <div class="overflow-x-auto" id="scrollable-data">
            <table class="min-w-full bg-white border border-gray-300">
                <thead>
                    ...
                    <!-- <th class="py-3 px-4 text-left border">Member ID</th> -->
                    <th class="py-3 px-4 text-left border">Member Name</th>
                        <th class="py-3 px-4 text-left border">Hotel ID</th>
                        <th class="py-3 px-4 text-left border">Hotel Name</th>
                        <th class="py-3 px-4 text-left border">Hotel Price</th>
                ...
                <tbody>
                    ...
                    if (!empty($_GET['order_number'])) {
                        $search_fields['o.order_number'] = $_GET['order_number'];
                    }
                    if (!empty($_GET['status'])) {
                        $search_fields['o.status'] = $_GET['status'];
                    }
                    if (!empty($_GET['order_date'])) {
                        $search_fields['o.order_date'] = $_GET['order_date'];
                    }
                    
                    $tbl_name = 'orders AS o JOIN users AS u ON o.user_id = u.id JOIN hotel_rooms AS p ON o.product_id = p.id';

                    $orders = get_records($tbl_name, $search_fields); 
                    ...
                    if (!empty($orders)) {
                        foreach ($orders as $order) {
                            ...
                            ?>
                            <tr>
                                <!-- <td class="py-3 px-4 border"><?php //echo $order['user_id']  ?></td> -->
                                <td class="py-3 px-4 border"><?php echo $order['username'] ?></td>
                                <td class="py-3 px-4 border"><?php echo $order['id']; ?></td>
                                <td class="py-3 px-4 border"><?php echo $order['hotel_name']; ?></td>
                                <td class="py-3 px-4 border">$<?php echo $order['price']; ?></td>
                                <td class="py-3 px-4 border"><?php echo $order['order_status']; ?></td>
