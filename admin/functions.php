...
function get_records($tbl_name, $search_fields = [], $num_per_page = 10)
{
    ...
    // Initialize an array to hold search conditions and values
    $search_conditions = [];
    $search_values = [];
    ...
    foreach ($search_fields as $field => $value) {
        if (!empty($value)) {
            // Sanitize input and decide on the operator based on the field
            $sanitized_value = mysqli_real_escape_string($conn, $value);

            if ($field === 'member_status') {
                // Use exact match for member_status
                $search_conditions[] = "$field = ?";
                $search_values[] = $sanitized_value;
            } else {
                // Use LIKE for other fields (e.g., partial matching)
    ...
    // Prepare the SELECT query with pagination
...
function get_luxury_order($tbl_name, $params = [], $search_fields = [], $num_per_page = 10)
{
  global $conn;

  // Set default values for keyword and max price if not provided
  $keyword = isset($params[0]) ? $params[0] : '';
  $maxPrice = isset($params[1]) ? $params[1] : 1000; // Default max price; adjust as needed

  $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
  $start_from = ($page - 1) * $num_per_page;

  // Initialize arrays for conditions and values
  $search_conditions = [];
  $search_values = [];
  ...
  foreach ($search_fields as $field => $value) {
    if (!empty($value)) {
      $search_conditions[] = "$field LIKE ?";
      $search_values[] = "%" . $value . "%"; // Partial matching
    }
  }
  ...
  // Bind parameters for count query if search values exist
  if (!empty($search_values)) {
    $types = str_repeat('s', count($search_values));
    mysqli_stmt_bind_param($count_stmt, $types, ...$search_values);
  }
  ...
  // Add start and limit values for pagination
  $params = array_merge($search_values, [$start_from, $num_per_page]);
  $types = str_repeat('s', count($search_values)) . 'ii';
...
function get_recharges($tbl_name, $search_fields = [], $num_per_page = 10)
{
  global $conn;
  ...
  // Count total records matching the search criteria
  $count_query = "SELECT COUNT(*) FROM $tbl_name AS r JOIN users AS u ON r.user_id = u.id $where_clause";
  $count_stmt = mysqli_prepare($conn, $count_query);
  ...
  $select_query = "SELECT 
    r.id AS recharge_id, 
    u.id AS user_id, 
    u.total_balance AS total_balance,
    u.username, 
    u.member_type, 
    r.recharge_amount, 
    r.status, 
    r.screenshot, 
    r.creation_time  
 FROM $tbl_name AS r JOIN 
    users AS u 
ON 
    r.user_id = u.id $where_clause LIMIT ?, ?";
  $select_stmt = mysqli_prepare($conn, $select_query);
...
function get_withdrawals($tbl_name, $search_fields = [], $num_per_page = 10)
{
  global $conn;
  ...
  // Count total records matching the search criteria
  $count_query = "SELECT COUNT(*) FROM $tbl_name AS w JOIN users AS u ON w.user_id = u.id $where_clause";
  $count_stmt = mysqli_prepare($conn, $count_query);
  ...
  $select_query = "SELECT 
    ...
    u.id AS user_id,
    u.total_balance AS total_balance,
    u.username AS member_account,
    w.withdrawal_amount AS withdrawal_amount,
    w.accountant_name AS account_title,
    w.account_number AS account_number,
    w.withdrawal_method AS method,
    w.withdrawal_status AS status,
    w.withdrawal_time AS withdrawal_time
FROM $tbl_name AS w
JOIN users AS u ON w.user_id = u.id
$where_clause
LIMIT ? , ?";
  $select_stmt = mysqli_prepare($conn, $select_query);
  // var_dump($select_stmt); exit();
