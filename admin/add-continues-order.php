...
?>
  ...
  if (isset($_GET['user_id'])) {
    ...
    // Check if POST request is made to update the luxury orders
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['confirm'])) {
        $luxury_order_id = $_POST['luxury-order-id'];  // Correct field name for form input
        $order_id = $_POST['order_id'];
        $luxury_commission = $_POST['luxury_commission'];

        // Prepare and execute the SQL update statement
        $stmt = $conn->prepare("UPDATE luxury_orders SET order_id = ?, luxury_commission = ? WHERE id = ?");
        $stmt->bind_param("iii", $order_id, $luxury_commission, $luxury_order_id);

        // Check for successful execution and return success/failure messages
        if ($stmt->execute()) {
            echo "<script>
                Swal.fire({
                    icon: 'success',
        ...
        <div id="continues-orders-container" class="flex pt-5 ">
          ...
          <div id="continues-orders-container-left">
            ...
            <div class="mb-4 ">
              ...
              <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300">* Commision on the Luxury Order
                </label>
              <input type="text" name="luxury_commission" class="mt-1 block w-full border border-zinc-300 dark:border-zinc-600 rounded-md p-2" />
            </div>
